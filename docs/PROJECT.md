...
- The whole project config has been built on a lie
- The system expected tables to be kept in order from the TOML config files, they are not
- The TOML spec does not state table order, the order can be undefined
- Array of Tables would work, but changes the TOML format
- Spec: https://github.com/toml-lang/toml/blob/main/toml.md#user-content-array-of-tables
- The go-toml package we are using does have an AST which maintains the order
